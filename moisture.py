...
GPIO.setup(first_channel, GPIO.IN)
GPIO.setup(second_channel, GPIO.IN)

# This line tells our script to keep an eye on our gpio pin and let us know when the pin goes HIGH or LOW
GPIO.add_event_detect(first_channel, GPIO.BOTH, bouncetime=300)
GPIO.add_event_detect(second_channel, GPIO.BOTH, bouncetime=300)
# This line asigns a function to the GPIO pin so that when the above line tells us there is a change on the pin, run this function
GPIO.add_event_callback(first_channel, callback)

# This is an infinte loop to keep our script running
while True:
	# This line simply tells our script to wait 0.1 of a second, this is so the script doesnt hog all of the CPU
	time.sleep(0.3)
	print "[17]: {0} | [27]: {1}".format(GPIO.input(first_channel), GPIO.input(second_channel))
